> model2_vs_model2_DRY.txt
> model2_vs_model2_OH2.txt

for name in $(ls /pub3/redo/docking-pp_e200k-110NaCl/e200k_110NaCl_model2_A-B/output_files/*.pdb)
do 
  filename="${name%.*}"
  ...
  echo $name $VALUE >> model2_vs_model2_DRY.txt
  VALUE=$(grep Pair "$filename"_OH2.out)
  echo $name $VALUE >> model2_vs_model2_OH2.txt
done 
