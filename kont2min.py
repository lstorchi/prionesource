...
if (len(sys.argv)) == 4: 
  kontname = sys.argv[1]
  MINDIM = int(sys.argv[2])
  NUMOFCLUST = int(sys.argv[3])
else:
  print "usage :", sys.argv[0] , " filename.kont mindim num_of_cluster"
  exit(1)
          ...
          for iy_near in range(-dimcube,dimcube):
            for iz_near in range(-dimcube,dimcube):
              ...
              if not ((ix_near == 0) and (iy_near == 0) and (iz_near == 0)):
                ...
                if (xl >= 0 and xl < nx) and (yl >= 0 and yl < ny) and \
                        (zl >= 0 and zl < nz) :
...
for j in range(0, NUMOFCLUST):
  ...
  if dim != 0.0:
      ...
      xval.append(centroids[j][0])
      yval.append(centroids[j][1])
      zval.append(centroids[j][2])
